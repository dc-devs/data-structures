...
describe('DoublyLinkedList', () => {
			...
			describe('with a valid index', () => {
				describe('and the index is equal to 0', () => {
					it('should insert a node at the begining of the linked list', () => {
						...
						expect(doublyLinkedList.head?.next).to.deep.equal(
							node1
						);
						expect(node1?.prev).to.deep.equal(
				...
				describe('and the index is equal to the length', () => {
					it('should insert a node at the end of the linked list', () => {
						...
						expect(doublyLinkedList.tail?.prev).to.be.equal(node3);
						expect(node3?.next).to.deep.equal(
			...
			describe('with a valid index', () => {
				...
				describe('and the index falls inside bounds of linked list', () => {
					it('should remove a node at index', () => {
						...
						const node1 = doublyLinkedList.get(1);
						const node3 = doublyLinkedList.get(3);
						const returnedNode = doublyLinkedList.remove(2);

						expect(
							returnedNode && returnedNode?.value
						).to.deep.equal(2);
						expect(
						...
						expect(node3?.prev).to.deep.equal(node1);
						expect(doublyLinkedList.length).to.be.equal(3);
					});
				});
			});
		...
		describe('when called', () => {
			describe('and linked list has nodes', () => {
				it('should unshift a new node onto the front of the linked list', () => {
					...
					expect(node1?.prev).to.deep.equal(doublyLinkedList.head);
				});
			});
		...
		describe('when called', () => {
			describe('and linked list has more than one nodes', () => {
				it('should shift a node off the front of the linked list', () => {
					...
					expect(shiftResult?.prev).to.be.equal(null);
	...
	describe('containsNodeWithValue', () => {
		it('exists', () => {
			const doublyLinkedList = new DoublyLinkedList(0);
			expect(typeof doublyLinkedList.containsNodeWithValue).to.be.equal(
				'function'
			);
		});

		describe('when called', () => {
			describe('and linked list has no nodes', () => {
				it('should return false', () => {
					const doublyLinkedList = new DoublyLinkedList(0);
					doublyLinkedList.pop();

					const result = doublyLinkedList.containsNodeWithValue(1);
					expect(result).to.be.false;
				});
			});

			describe('and the value is in the list', () => {
				describe('and linked list has one node', () => {
					it('should return true', () => {
						const doublyLinkedList = new DoublyLinkedList(0);

						const result =
							doublyLinkedList.containsNodeWithValue(0);
			...
			describe('and the value is not in the list', () => {
				describe('and linked list has one node', () => {
					it('should return false', () => {
						const doublyLinkedList = new DoublyLinkedList(0);
